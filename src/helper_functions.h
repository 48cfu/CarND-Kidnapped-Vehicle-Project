...
struct LandmarkObs {
  ...
  operator Map::single_landmark_s() const {
    Map::single_landmark_s single{};
    single.id_i = id;
    single.x_f = static_cast<float>(x);
    single.y_f = static_cast<float>(y);
    return single;
  }
};

/* Computes the particle weights with a multi-variate Gaussian distribution
*/
